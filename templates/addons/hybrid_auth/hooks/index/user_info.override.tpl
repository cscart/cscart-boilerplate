...
{if !$auth.user_id}
    <a id="sw_login" {if $settings.General.secure_storefront == "partial"} rel="nofollow" href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}{$config.current_url|fn_url}{else}{"auth.login_form?return_url=`$escaped_current_url`"|fn_url}{/if}"{else}class="cm-combination combination-link"{/if}>{__("sign_in")}</a>
{else}
...
{if $settings.General.secure_storefront != "partial"}
