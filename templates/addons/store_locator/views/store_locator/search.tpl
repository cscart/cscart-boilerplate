    ...
    <div class="store-location row">
        ...
        <div class="wysiwyg-content store-location-locations-wrapper col-lg-4 col-md-4 col-sm-4">
            ...
            {foreach from=$store_locations item=loc key=num name="locations"}
                <a href="#" class="list-group-item cm-map-view-location {if $smarty.foreach.locations.first}active{/if}" id="loc_{$loc.store_location_id}" data-ca-latitude={$loc.latitude} data-ca-longitude={$loc.longitude}>
            ...
            {if $store_locations|count > 0}
                <a href="#" class="list-group-item cm-map-view-locations">{__("all_stores")}</a>
            {/if}
            </div>
        </div>
    </div>
