            ...
            <tr>
                ...
                {if $feature.prefix && !$hide_affix}<span class="product-feature-prefix">{$feature.prefix}</span>{/if}
                    ...
                    {elseif $feature.feature_type == "ProductFeatures::MULTIPLE_CHECKBOX"|enum && $feature.variants}
                        <ul class="product-feature-multiple">
                        {foreach from=$feature.variants item="var"}
                            {assign var="hide_variant_affix" value=!$hide_affix}
                            {if $var.selected}<li class="product-feature-multiple-item"><span class="compare-checkbox" title="{$var.variant}"><i class="glyphicon glyphicon-check fa fa-check-square-o"></i></span>{if !$hide_variant_affix}<span class="product-feature-prefix">{$feature.prefix}</span>{/if}{$var.variant}{if !$hide_variant_affix}<span class="product-feature-suffix">{$feature.suffix}</span>{/if}</li>{/if}
                        {/foreach}
                        </ul>
